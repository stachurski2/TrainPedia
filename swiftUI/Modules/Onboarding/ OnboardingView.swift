...
struct OnboardingInnerView: View {
    ...
    var body: some View {
        VStack(alignment: .center, spacing: 5.0, content: {
            Text("TrainPedia").font(.system(size: 50)).padding(EdgeInsets(top: 0, leading: 0, bottom: kStandardInset, trailing: 0))
                        Image("onboardingImage").resizable(capInsets: EdgeInsets(top: 0, leading: 0, bottom: 0, trailing: 0), resizingMode: .stretch).aspectRatio(contentMode: .fit).clipShape(Circle()).padding(.horizontal, 15.0).rotationEffect(Angle(degrees: 45)).padding(EdgeInsets(top: 0, leading: 0, bottom: kStandardInset, trailing: 0))
                            Divider()
